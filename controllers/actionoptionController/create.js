const actionOptions = require('@/models/actionOptions');

const createMenuOptions = async (req, res) => {
  try {
    // Check if menuOptions for the provided role already exist
    const existingMenuOptions = await actionOptions.findOne({ role: req.body.role });
    ...
    const newMenuOptions = new ˇ(req.body);
